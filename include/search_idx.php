...
function update_search_index($mode, $post_id, $message, $subject = null)
{
	...
	if ($mode == 'edit')
	{
		$result = $db->query('SELECT w.id, w.word, m.subject_match FROM '.$db_prefix.'search_words AS w INNER JOIN '.$db_prefix.'search_matches AS m ON w.id=m.word_id WHERE m.post_id='.$post_id, true) or error('Unable to fetch search index words', __FILE__, __LINE__, $db->error());
	...
	if (!empty($unique_words))
	{
		...
		if (!empty($new_words))
		{
			switch ($db_type)
			{
				case 'mysql':
				case 'mysqli':
					$db->query('INSERT INTO '.$db_prefix.'search_words (word) VALUES'.implode(',', preg_replace('#^(.*)$#', '(\'\1\')', $new_words))) or error('Unable to insert search index words', __FILE__, __LINE__, $db->error());
					break;

				default:
					while (list(, $word) = @each($new_words))
						$db->query('INSERT INTO '.$db_prefix.'search_words (word) VALUES(\''.$word.'\')') or error('Unable to insert search index words', __FILE__, __LINE__, $db->error());
					break;
			}
		}
	...
	while (list($match_in, $wordlist) = @each($words['del']))
	{
		...
		if (!empty($wordlist))
		{
			...
			$db->query('DELETE FROM '.$db_prefix.'search_matches WHERE word_id IN('.$sql.') AND post_id='.$post_id.' AND subject_match='.$subject_match) or error('Unable to delete search index word matches', __FILE__, __LINE__, $db->error());
		}
	}
	...
	while (list($match_in, $wordlist) = @each($words['add']))
	{
		...
		if (!empty($wordlist))
			$db->query('INSERT INTO '.$db_prefix.'search_matches (post_id, word_id, subject_match) SELECT '.$post_id.', id, '.$subject_match.' FROM '.$db_prefix.'search_words WHERE word IN('.implode(',', preg_replace('#^(.*)$#', '\'\1\'', $wordlist)).')') or error('Unable to insert search index word matches', __FILE__, __LINE__, $db->error());
	}
...
function strip_search_index($post_ids)
{
	...
	switch ($db_type)
	{
		...
		case 'mysqli':
		{
			$result = $db->query('SELECT word_id FROM '.$db_prefix.'search_matches WHERE post_id IN('.$post_ids.') GROUP BY word_id') or error('Unable to fetch search index word match', __FILE__, __LINE__, $db->error());
			...
			if ($result->fetchRow(MDB2_FETCHMODE_ASSOC))
			{
				...
				$result = $db->query('SELECT word_id FROM '.$db_prefix.'search_matches WHERE word_id IN('.$word_ids.') GROUP BY word_id HAVING COUNT(word_id)=1') or error('Unable to fetch search index word match', __FILE__, __LINE__, $db->error());
				...
				if ($result->fetchRow(MDB2_FETCHMODE_ASSOC))
				{
					...
					$db->query('DELETE FROM '.$db_prefix.'search_words WHERE id IN('.$word_ids.')') or error('Unable to delete search index word', __FILE__, __LINE__, $db->error());
				}
			}
		...
		default:
			$db->query('DELETE FROM '.$db_prefix.'search_words WHERE id IN(SELECT word_id FROM '.$db_prefix.'search_matches WHERE word_id IN(SELECT word_id FROM '.$db_prefix.'search_matches WHERE post_id IN('.$post_ids.') GROUP BY word_id) GROUP BY word_id HAVING COUNT(word_id)=1)') or error('Unable to delete from search index', __FILE__, __LINE__, $db->error());
			break;
	}

	$db->query('DELETE FROM '.$db_prefix.'search_matches WHERE post_id IN('.$post_ids.')') or error('Unable to delete search index word match', __FILE__, __LINE__, $db->error());
}
