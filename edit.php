...
$result = $db->query('SELECT f.id AS fid, f.forum_name, f.moderators, f.redirect_url, fp.post_replies, fp.post_topics, t.id AS tid, t.subject, t.posted, t.closed, p.poster, p.poster_id, p.message, p.hide_smilies FROM '.$db_prefix.'posts AS p INNER JOIN '.$db_prefix.'topics AS t ON t.id=p.topic_id INNER JOIN '.$db_prefix.'forums AS f ON f.id=t.forum_id LEFT JOIN '.$db_prefix.'forum_perms AS fp ON (fp.forum_id=f.id AND fp.group_id='.$pun_user['g_id'].') WHERE (fp.read_forum IS NULL OR fp.read_forum=1) AND p.id='.$id) or error('Unable to fetch post info', __FILE__, __LINE__, $db->error());
if (!$result->numRows($result))
	message($lang_common['Bad request']);

$cur_post = $result->fetchRow(MDB2_FETCHMODE_ASSOC);
...
$result = $db->query('SELECT id FROM '.$db_prefix.'posts WHERE topic_id='.$cur_post['tid'].' ORDER BY posted LIMIT 1') or error('Unable to fetch post info', __FILE__, __LINE__, $db->error());
$topic_post_id = $result->fetchRow(MDB2_FETCHMODE_ASSOC);
...
if (isset($_POST['form_sent']))
{
	...
	if (empty($errors) && !isset($_POST['preview']))
	{
		...
		if ($can_edit_subject)
		{
			...
			$db->query('UPDATE '.$db_prefix.'topics SET subject=\''.$db->escape($subject).'\' WHERE id='.$cur_post['tid'].' OR moved_to='.$cur_post['tid']) or error('Unable to update topic', __FILE__, __LINE__, $db->error());
		...
		// Update the post
		$db->query('UPDATE '.$db_prefix.'posts SET message=\''.$db->escape($message).'\', hide_smilies=\''.$hide_smilies.'\''.$edited_sql.' WHERE id='.$id) or error('Unable to update post', __FILE__, __LINE__, $db->error());
